import 'package:badges/badges.dart';
import 'package:ecommerce_app_eid/constants/screens_ids.dart';
import 'package:ecommerce_app_eid/controllers/cart_controller.dart';
import 'package:ecommerce_app_eid/controllers/favourite_controller.dart';
import 'package:ecommerce_app_eid/localization/language_constants.dart';
import 'package:ecommerce_app_eid/screens/shopping_cart.dart';
import 'package:ecommerce_app_eid/skeletons/Product_detail_skeleton.dart';
import 'package:ecommerce_app_eid/widgets/cart_button.dart';
import 'package:ecommerce_app_eid/widgets/product_detail_bottom_sheet_content.dart';
import 'package:ecommerce_app_eid/widgets/round_cart_button.dart';
import 'package:flutter/material.dart';
import 'package:provider/provider.dart';
import 'package:shimmer/shimmer.dart';

class ProductDetail extends StatefulWidget {
  ProductDetail({Key key}) : super(key: key);
  static String id = ProductDetail_Screen_Id;

  @override
  _ProductDetailState createState() => _ProductDetailState();
}

class _ProductDetailState extends State<ProductDetail> {
  var _cartCtlr;

  var _cartfavouriteCtrl;

  // bool favourite = false;
  //SharedPreferences prefs;
  // FavoriteProductsDbHelper favoriteProductsDbHelper;
  bool favorite = false;

//******************************************************************************
  /* Future<void> getFavoriteBool() async {
    bool favorite = await favoriteProductsDbHelper.favoriteProductExists(
      int.parse(_cartCtlr.selectedItem.product.id),
      _cartCtlr.selectedItem.product.name,
    );
    setState(() {
      this.favorite = favorite ?? false;
    });
  }*/
//******************************************************************************
  /* saveFavourtie(bool fav) async {
    prefs = await SharedPreferences.getInstance();
    prefs.setBool('aa', fav);
  }

  getfavourite() async {
    prefs = await SharedPreferences.getInstance();
    setState(() {
      favorite = prefs.getBool('aa') ?? false;
    });
  }*/

//******************************************************************************
  @override
  void initState() {
    super.initState();
    _cartCtlr = Provider.of<CartController>(context, listen: false);

    //getFavoriteBool();
    //favoriteProductsDbHelper = FavoriteProductsDbHelper();

    _cartfavouriteCtrl =
        Provider.of<FavouriteController>(context, listen: false);

    //getfavourite();
  }

  //****************************************************************************
  _handleButtonTap(context) {
    if (!_cartCtlr.isItemInCart(_cartCtlr.selectedItem)) {
      _cartCtlr.addToCart(_cartCtlr.selectedItem);
    } else {
      showModalBottomSheet(
        context: context,
        builder: (BuildContext context) =>
            ProductDetailBottomSheetContent(cartCtlr: _cartCtlr),
      );
    }
  }

  //****************************************************************************
  _handleButtonFavourite(context) {
    if (!_cartfavouriteCtrl
        .isItemInFavouriteCart(_cartfavouriteCtrl.selectedItem)) {
      _cartfavouriteCtrl.addToFavouriteCart(_cartfavouriteCtrl.selectedItem);
    } else {
      _cartfavouriteCtrl
          .removeFromFavouriteCart(_cartfavouriteCtrl.selectedItem);
    }
  }

  /* Future<void> getFavoriteBool() {
    bool favorite = _cartfavouriteCtrl
        .isItemInFavouriteCart(_cartfavouriteCtrl.selectedItem);
    setState(() {
      this.favorite = favorite ?? false;
    });
  }*/

  /* void _save() {
    setState(() {
      if (favourite) {
        favourite = false;
      } else {
        favourite = true;
      }
    });
    //saveFavourtie(favourite);
    _handleButtonFavourite(context);
  }*/

  //****************************************************************************
  _handleQuantityIncrease() {
    _cartCtlr.increaseCartItemAndProductDetailItemQuantity();
  }

  //****************************************************************************
  _handleQuantityDecrease() {
    _cartCtlr.decreaseCartItemAndProductDetailItemQuantity();
  }

  //****************************************************************************
  @override
  Widget build(BuildContext context) {
    var size = MediaQuery.of(context).size;
    double _leftMargin = 10;
    double _rightMargin = 10;
    return Scaffold(
      appBar: AppBar(
        iconTheme: IconThemeData(color: Colors.black),
        elevation: 0,
        backgroundColor: Colors.white,
        title: Consumer<CartController>(
          builder: (context, cartCtlr, child) {
            if (!cartCtlr.isLoadingProduct) {
              return Text(
                "${cartCtlr.selectedItem.product.category}",
                style: TextStyle(
                  color: Colors.black,
                  fontSize: 30,
                ),
              );
            }
            return Text('Loading ...');
          },
        ),
        actions: [
          Container(
            margin: EdgeInsets.only(right: 20, top: _rightMargin),
            child: GestureDetector(
              onTap: () {
                Navigator.pushNamed(context, ShoppingCart.id);
              },
              child: Badge(
                padding: EdgeInsets.all(5),
                badgeContent: Text(
                  '${context.watch<CartController>().cart.length}',
                  style: TextStyle(
                    color: Colors.white,
                  ),
                ),
                child: Icon(
                  Icons.shopping_cart,
                  color: Colors.orange,
                ),
              ),
            ),
          ),
        ],
      ),
      body: SafeArea(
        child: Consumer<CartController>(
          builder: (context, cartCtlr, child) {
            if (cartCtlr.isLoadingProduct) {
              return Center(
                child: Shimmer.fromColors(
                  child: ProductDetailSkeleton(),
                  baseColor: Colors.grey[300],
                  highlightColor: Colors.grey[400],
                ),
              );
            } else {
              return ListView(
                children: [
                  Container(
                    margin: EdgeInsets.only(
                      left: _leftMargin,
                      right: _rightMargin,
                    ),
                    child: Column(
                      children: [
                        //product image
                        Container(
                          height: size.width / 2 + 100,
                          width: size.width,
                          child: Image.network(
                            cartCtlr.selectedItem.product.imageUrl,
                            fit: BoxFit.fill,
                            errorBuilder: (BuildContext context,
                                Object exception, StackTrace stackTrace) {
                              return Center(child: Icon(Icons.error));
                            },
                            loadingBuilder: (BuildContext context, Widget child,
                                ImageChunkEvent loadingProgress) {
                              if (loadingProgress == null) return child;
                              return Center(
                                child: CircularProgressIndicator(
                                  strokeWidth: 2.0,
                                ),
                              );
                            },
                          ),
                        ),

                        SizedBox(
                          height: 30,
                        ),

                        //product name and price
                        Padding(
                          padding: const EdgeInsets.all(8.0),
                          child: Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              Text(
                                "${cartCtlr.selectedItem.product.name}",
                                style: TextStyle(
                                    fontWeight: FontWeight.bold, fontSize: 15),
                              ),
                              Text(
                                "\$${cartCtlr.selectedItem.product.price}",
                                style: TextStyle(fontSize: 15),
                              ),
                            ],
                          ),
                        ),

                        Divider(
                          thickness: 3,
                        ),
                        // increment and decrement buttons, add to cart button and favourtie button
                        Container(
                          color: Colors.grey[200],
                          child: Padding(
                            padding: const EdgeInsets.all(8.0),
                            child: Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: [
                                // quantity ,increment,and decrement buttons
                                Row(
                                  children: [
                                    //decrement button
                                    RoundCartButton(
                                      icon: Icons.remove,
                                      width: size.width * 0.1,
                                      onTap: () {
                                        _handleQuantityDecrease();
                                      },
                                    ),
                                    Padding(
                                      padding: const EdgeInsets.all(8.0),
                                      // quantity
                                      child: Text(
                                        '${context.watch<CartController>().selectedItem.quantity}',
                                        style: TextStyle(
                                          fontWeight: FontWeight.bold,
                                        ),
                                      ),
                                    ),
                                    // increment button
                                    RoundCartButton(
                                      icon: Icons.add,
                                      width: size.width * 0.1,
                                      onTap: () {
                                        _handleQuantityIncrease();
                                      },
                                    ),
                                  ],
                                ),

                                /*
                                // add to favourite cart
                                IconButton(
                                  iconSize: 40,
                                  icon: favorite
                                      ? Icon(Icons.favorite)
                                      : Icon(Icons.favorite_border),
                                  color: favorite ? Colors.red : Colors.black,
                                  onPressed: () async {
                                    if (favorite) {
                                      await favoriteProductsDbHelper.delete(
                                        int.parse(
                                            cartCtlr.selectedItem.product.id),
                                        cartCtlr.selectedItem.product.name,
                                      );
                                    } else {
                                      FavoriteProduct newFavoriteProduct =
                                          FavoriteProduct(
                                        id: int.parse(
                                            cartCtlr.selectedItem.product.id),
                                        name:
                                            cartCtlr.selectedItem.product.name,
                                        price:
                                            cartCtlr.selectedItem.product.price,
                                        imageUrl: cartCtlr
                                            .selectedItem.product.imageUrl,
                                        category: cartCtlr
                                            .selectedItem.product.category,
                                        details: cartCtlr
                                            .selectedItem.product.details,
                                        v: cartCtlr.selectedItem.product.v,
                                      );
                                      await favoriteProductsDbHelper
                                          .add(newFavoriteProduct);
                                    }
                                    setState(() {
                                      favorite = !favorite;
                                    });
                                  },
                                ),*/

                                /* IconButton(
                                  icon: favorite
                                      ? Icon(Icons.favorite)
                                      : Icon(Icons.favorite_border),
                                  color: favorite ? Colors.red : Colors.black,
                                  onPressed: () {
                                    if (favorite) {
                                      _cartfavouriteCtrl
                                          .removeFromFavouriteCart(
                                        _cartfavouriteCtrl.selectedItem,
                                      );
                                    } else {
                                      if (!_cartfavouriteCtrl
                                          .isItemInFavouriteCart(
                                              _cartfavouriteCtrl
                                                  .selectedItem)) {
                                        _cartfavouriteCtrl.addToFavouriteCart(
                                          _cartfavouriteCtrl.selectedItem,
                                        );
                                      }
                                    }
                                    setState(() {
                                      favorite = !favorite;
                                    });
                                  },
                                ),*/

                                InkWell(
                                  onTap: () {
                                    _handleButtonFavourite(context);
                                  },
                                  child: CartButton(
                                    text: 'favourit',
                                    width: size.width * 0.2,
                                  ),
                                ),
                                //add to cart button
                                InkWell(
                                  onTap: () {
                                    _handleButtonTap(context);
                                  },
                                  child: CartButton(
                                    text: getTranslated(context, 'ADD_btn'),
                                    width: size.width * 0.2,
                                  ),
                                )
                              ],
                            ),
                          ),
                        ),
                        Divider(
                          thickness: 3,
                        ),

                        SizedBox(
                          height: 30,
                        ),
                        //Details
                        Text(
                          getTranslated(context, 'Details_tex'),
                          style: TextStyle(
                            fontWeight: FontWeight.bold,
                            fontSize: 25,
                          ),
                        ),
                        SizedBox(
                          height: 10,
                        ),
                        Text(
                          "${cartCtlr.selectedItem.product.details}",
                          style: TextStyle(
                            fontWeight: FontWeight.w500,
                          ),
                          overflow: TextOverflow.clip,
                          textAlign: TextAlign.justify,
                        ),

                        SizedBox(
                          height: 30,
                        ),
                      ],
                    ),
                  ),
                ],
              );
            }
          },
        ),
      ),
    );
  }
}
